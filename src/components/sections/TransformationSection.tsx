...
export default function TransformationSection() {
  ...
  return (
    <section className="bg-brand-secondary py-24 md:py-32 relative overflow-hidden border-y border-sage-soft/10">
            ...
            {stages.map((stage, i) => (
              <div key={i} className="flex-1 flex flex-col lg:items-center lg:text-center relative group">
                <div className="w-[68px] h-[68px] md:w-[80px] md:h-[80px] rounded-full bg-white border border-sage-soft/15 shadow-sm flex items-center justify-center relative mb-6 lg:mb-8 z-30 transition-all duration-500 group-hover:scale-105 group-hover:border-sage-soft group-hover:shadow-md shrink-0">
                  <div className="absolute inset-0 rounded-full bg-sage-soft/10 group-hover:scale-125 group-hover:opacity-0 transition-all duration-700" />
                ...
                {i < stages.length - 1 && (
                  <div className="absolute left-[34px] md:left-[40px] top-[74px] md:top-[86px] w-[1px] h-[72px] bg-sage-soft/30 lg:hidden" aria-hidden="true" />
                )}
              </div>
            ))}
          </div>
